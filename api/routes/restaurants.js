// src/routes/restaurants.js — Public restaurant routes
'use strict';
...
router.get('/', apiLimiter, async (req, res, next) => {
  try {
    ...
    if (search) {
      params.push(`%${search}%`);
      where += ` AND (r.name ILIKE $${params.length} OR r.address ILIKE $${params.length})`;
    }
    if (cuisine) {
      params.push(cuisine);
      where += ` AND $${params.length} = ANY(r.cuisine)`;
    }
    if (price) {
      params.push(price);
      where += ` AND r.price_category = $${params.length}`;
    }
    if (premium === 'true') {
      where += ` AND r.is_premium = true`;
    }

    params.push(parseInt(limit), parseInt(offset));
    const { rows } = await pool.query(
      `SELECT r.id, r.name, r.description, r.address, r.phone,
              r.cuisine, r.price_category, r.rating, r.review_count,
              r.image_url, r.working_hours, r.latitude, r.longitude,
              r.capacity, r.is_premium, r.status
       FROM restaurants r
       ${where}
       ORDER BY r.is_premium DESC, r.rating DESC, r.review_count DESC
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM restaurants WHERE id = $1 AND status = 'approved'`, [id]
    );
...
router.get('/:id/availability', async (req, res, next) => {
  try {
    ...
    // Also include manually blocked times
...
router.get('/:id/reviews', async (req, res, next) => {
  try {
    const id = parseInt(req.params.id);
    const { rows } = await pool.query(
      `SELECT rv.id, rv.rating, rv.comment, rv.photo_url, rv.created_at,
