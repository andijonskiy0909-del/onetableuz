...
function registerUploadRoutes(app, authMiddleware) {
  const middleware = authMiddleware || ((req, res, next) => next())

  app.post('/api/uploads/image', middleware, (req, res) => {
  ...
  app.post('/api/uploads/video', middleware, (req, res) => {
    upload.single('file')(req, res, async err => {
      if (err) return res.status(400).json({ error: err.message })
      if (!req.file) return res.status(400).json({ error: 'Fayl yuborilmadi' })
      try {
  ...
  app.delete('/api/uploads/:file_id', middleware, async (req, res) => {
    try {
