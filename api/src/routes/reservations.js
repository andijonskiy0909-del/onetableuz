...
router.post('/', userAuth, validateReservation, async (req, res) => {
  ...
  try {
    ...
    // Restoran mavjudligini tekshirish - Race condition oldini olish uchun FOR UPDATE shu yerga qo'shildi
    const resto = await client.query(
      'SELECT id, name, capacity FROM restaurants WHERE id = $1 AND status = $2 FOR UPDATE',
    ...
    // Konflikt tekshiruvi (FOR UPDATE bu yerdan olib tashlandi, chunki COUNT bilan ishlamaydi)
...
// Qolgan GET va DELETE funksiyalari o'zgarishsiz qoladi...
