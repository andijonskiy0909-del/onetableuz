...
exports.createRestaurant = asyncHandler(async (req, res) => {
  ...
  `, [
    ...
    b.address || null,
    b.city || 'Toshkent',
    b.working_hours || '10:00-23:00',
    Number(b.capacity) || 50,
    b.price_category || '$$',
    b.image_url || null,
    b.cover_url || null,
    Array.isArray(b.cuisine)
      ? b.cuisine
      : (b.cuisine ? b.cuisine.split(',').map(s => s.trim()).filter(Boolean) : []),
    Array.isArray(b.gallery) ? b.gallery : [],
    Boolean(b.deposit_required),
    Number(b.deposit_amount) || 0,
    Number(b.min_guests) || 1,
    Number(b.max_guests) || 20,
    Boolean(b.has_parking),
    Boolean(b.has_wifi),
    Boolean(b.has_kids_area),
    Boolean(b.has_outdoor),
    Boolean(b.has_live_music)
  ])

  await db.query(
    'UPDATE owners SET restaurant_id = $1 WHERE id = $2',
    [r.rows[0].id, req.owner.id]
  )
...
exports.updateRestaurant = asyncHandler(async (req, res) => {
  ...
  if (b.cuisine !== undefined) {
    const arr = Array.isArray(b.cuisine)
      ? b.cuisine
      : (b.cuisine ? b.cuisine.split(',').map(s => s.trim()).filter(Boolean) : [])
    push('cuisine', arr)
  }
...
async function getOwnerReservationNotifyDetails(reservationId) {
  const r = await db.query(`
    SELECT 
      res.id,
      res.date,
...
async function sendOwnerReservationStatusMessage(reservationId, status) {
  try {
    const booking = await getOwnerReservationNotifyDetails(reservationId)

    if (!booking) {
      console.warn('[Telegram] Bron topilmadi')
      return
    }

    if (!booking.telegram_id) {
      console.warn('[Telegram] Mijoz telegram_id topilmadi')
      return
    }
...
Restoran bronni bekor qildi. Boshqa vaqt tanlab qayta bron qilishingiz mumkin.
`
    }

    if (!message) return

    await sendTelegramMessage(booking.telegram_id, message)
  } catch (err) {
    console.error('[Telegram] Dashboard status xabari yuborilmadi:', err.message)
  }
}
...
exports.updateReservation = asyncHandler(async (req, res) => {
  ...
  if (!validStatuses.includes(status)) {
    throw AppError.badRequest('Yaroqsiz holat')
  }
  ...
  const r = await db.query(`
    ...
    WHERE id = $${pi++} AND restaurant_id = $${pi}
    RETURNING *
  `, params)

  if (!r.rows.length) {
    throw AppError.notFound('Bron topilmadi')
  }
  ...
  if (io) {
    io.to(`restaurant_${req.owner.restaurant_id}`).emit('reservation_updated', reservation)
  }

  res.json(reservation)

  if (status === 'confirmed' || status === 'cancelled') {
    sendOwnerReservationStatusMessage(reservation.id, status).catch(err => {
      console.error('[Telegram] Dashboard notification error:', err.message)
    })
  }
})
...
exports.listZones = asyncHandler(async (req, res) => {
  ...
  const r = await db.query(
    'SELECT * FROM zones WHERE restaurant_id = $1 ORDER BY sort_order NULLS LAST, id',
    [req.owner.restaurant_id]
  )
...
exports.listReels = asyncHandler(async (req, res) => {
  ...
  const r = await db.query(
    'SELECT * FROM reels WHERE restaurant_id = $1 ORDER BY created_at DESC',
    [req.owner.restaurant_id]
  )
...
exports.deleteReel = asyncHandler(async (req, res) => {
  await db.query(
    'DELETE FROM reels WHERE id = $1 AND restaurant_id = $2',
    [req.params.id, req.owner.restaurant_id]
  )
...
exports.analytics = asyncHandler(async (req, res) => {
  if (!req.owner.restaurant_id) {
    return res.json({ today: 0, weekly: 0, monthly: 0, revenue: 0 })
  }
  ...
  const [today, week, month, revenue, topZones, hourly] = await Promise.all([
    db.query(
      `SELECT COUNT(*)::int AS c FROM reservations WHERE restaurant_id=$1 AND date=CURRENT_DATE AND status NOT IN ('cancelled')`,
      [rid]
    ),
    db.query(
      `SELECT COUNT(*)::int AS c FROM reservations WHERE restaurant_id=$1 AND date >= CURRENT_DATE - 7 AND status NOT IN ('cancelled')`,
      [rid]
    ),
    db.query(
      `SELECT COUNT(*)::int AS c FROM reservations WHERE restaurant_id=$1 AND date >= CURRENT_DATE - 30 AND status NOT IN ('cancelled')`,
      [rid]
    ),
    db.query(
      `SELECT COALESCE(SUM(pre_order_total),0)::numeric AS s FROM reservations WHERE restaurant_id=$1 AND date=CURRENT_DATE AND status NOT IN ('cancelled')`,
      [rid]
    ),
    db.query(`
    ...
    db.query(`
      ...
      FROM reservations
      WHERE restaurant_id=$1 AND date >= CURRENT_DATE - 30 AND status NOT IN ('cancelled')
      GROUP BY hour
      ORDER BY hour
    `, [rid])
  ])
...
exports.premiumStatus = asyncHandler(async (req, res) => {
  ...
  const r = await db.query(
    'SELECT is_premium, premium_until FROM restaurants WHERE id = $1',
    [req.owner.restaurant_id]
  )
