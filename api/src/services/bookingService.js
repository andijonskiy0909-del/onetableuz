...
// ── Find available table (with row-level locking) ──
async function findAvailableTable(client, restaurantId, zoneId, date, time, guests) {
  ...
  // If another transaction is currently locking this table, skip it
  q += ' ORDER BY t.capacity ASC LIMIT 1 FOR UPDATE SKIP LOCKED'
...
async function createReservation(userId, data, io) {
  ...
  try {
    ...
    const userCheck = await client.query(
      'SELECT trust_score, noshow_count FROM users WHERE id = $1',
      [userId]
    )
    if (userCheck.rows.length) {
      const trustScore = userCheck.rows[0].trust_score ?? 100
      if (trustScore < 20) {
        throw AppError.forbidden('Sizning ishonch reytingingiz juda past. Bron qilish mumkin emas.')
      }
    }
    ...
    // Increment user's total_bookings
    db.query(
      'UPDATE users SET total_bookings = COALESCE(total_bookings, 0) + 1 WHERE id = $1',
      [userId]
    ).catch(() => {})
    ...
    db.query(
      `INSERT INTO activity_log (actor_type, actor_id, action, entity_type, entity_id, metadata)
       VALUES ('user', $1, 'reservation_created', 'reservation', $2, $3)`,
      [userId, reservation.id, JSON.stringify({ restaurant_id, date, time, guests })]
    ).catch(() => {})
