...
const i18n = {
  uz: {
    ...
    review_ask: n => `⭐️ <b>${n}</b> ga qanday baho berasiz?`,
  ...
  ru: {
    ...
    btn_ai: '🤖 AI Ассистент',
    help: '📱 Команды:\n/start — Старт\n/mybookings — Брони\n/ai — AI ассистент\n/help — Помощь',
    review_ask: n => `⭐️ Как вы оцениваете <b>${n}</b>?`,
  ...
  en: {
    ...
    review_ask: n => `⭐️ How would you rate <b>${n}</b>?`,
...
[07.04.2026 22:39] Мухаммадали: return typeof v === 'function' ? v(...args) : (v  key)
}
...
[07.04.2026 22:39] Мухаммадали: if (state.ai[uid] && msg.text) { await handleAI(uid, chatId, msg.text); return }

  const rs = state.reviews[uid]
  if (!rs) return

  if (rs.step === 'comment' && msg.text) {
    rs.comment = msg.text; rs.step = 'photo'
    bot.sendMessage(chatId, T(uid, 'review_photo'), {
      reply_markup: { inline_keyboard: [[{ text: T(uid, 'review_skip'), callback_data: 'review_skip_photo' }]] }
    })
    return
...
async function saveReview(uid, chatId, photoUrl) {
  const rs = state.reviews[uid]; if (!rs) return
  try {
...
[07.04.2026 22:39] Мухаммадали: // ── Review so'rash ────────────────────────────────────────────
